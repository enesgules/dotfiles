...
IMPORTANT: Resolve at most twice per question (`ctx7 library`). Once you have a library ID, you may issue one `ctx7 docs` query per distinct concept the question covers — decompose multi-topic questions rather than retrying a single combined query. If you still cannot find what you need after a few targeted `docs` queries, use the best result you have.
...
| Bad (too vague) | `"auth"` |
| Bad (too vague) | `"hooks"` |
| Bad (too broad) | `"routing and auth and caching and forms in Next.js"` |

Use the user's full question as the query when possible — vague one-word queries return generic results. But keep each query to a *single* intent: if the question spans several distinct concepts, split it into one `ctx7 docs` query per concept (reusing the same library ID) instead of cramming them all into one. Context7 ranks against a single intent, so a combined query dilutes ranking and returns shallow results for each topic.
